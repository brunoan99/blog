...
Começando da forma mais simples possível Estruturas de Dados são formas de se organizar dados para obter um comportamento ou atingir uma eficiência maior em comparação com um padão "desorganizado".
...
Apesar de um conceito abstrato e um pouco genérico podemos definir Estruturas de Dados pelo forma como dados são armazenados e tratados, através das rotinas ou métodos da estrutura em questão. Apesar da abastração conseguimos agrupar Estruturas de Dados que nos ajudam a compreender seu comportamento.

//REDO
Cada Estrutura é particularmente diferente das demais
Possibilidade de agrupar comportamentos em diferentes rótulos
...
Imagino eu que todo e qualquer programador ou pessoa que esteja lendo esse post tenha uma rasa noção a respeito de Alocação de memória. Caso não, recomendo leitura.

No que diz respeito a Alocação de Memória em Estrutura de Dados pode-mos dividir em duas as diferentes formas:
- Alocação Estático ou Tamanho Estático;
...
Alocação Estática resume-se a utilizar previamente do conhecimento do tamanho de um dado e realizar alocação sequêncial de memória suficiente para conter um arranjo destes dados desejados. Um claro exemplo são os Arrays.
...
Exemplificando de forma rasa, supondo que desejamos instanciar um Array do tipo **u32**, esperando que o Array abrigue 5 posições deste tipo de dado. Se para cada posição **precisamos** de 4 bytes, então basta pedir ao sistema operacional por uma porção de memória de 20 bytes para este Array e teremos espaço suficiente.

Vamos simular. Como seria alocar um Array e definir valores para cada item dele. Neste exemplo temos os 20 bytes alocados, entenda cada bloco abaixo como um conjunto de **4 bytes**, cada um para representar um valor no Array.
...
Depende. Há inumeras variáveis que podem determinar o resultado dessa tentativa, podendo ser deste a linguagem utilizada até mesmo o sistema em que o processo irá executar.
...
// Linearidade
// exemplo Procurar número em um linear outro n linear
